...
const countHours = (year: number, holidays: string[]): number => {
  const dates = holidays
    .map((element) => new Date(year + '/' + element))
    .map((element) => element.getDay())

  return dates.reduce(
    (acc, current) => (current !== 0 && current !== 6 ? (acc = acc + 2) : acc),
    0
  )
}
